...
export default function IncomeList() {
  ...
  const handleDelete = (itemId) => {
    const deleteEntries = entries.filter((entry) => entry.id !== itemId);
    setEntries(deleteEntries);
  };

  const handleEdit = (itemId) => {
    const inputValue = prompt("Enter new value");
            ...
            <li key={income.id} className="py-2.5">
              <div className="group flex justify-between gap-2 text-sm">
                ...
                <div>
                  ...
                  <span
                    className="ml-2 hidden cursor-pointer font-medium text-green-600 group-hover:inline-block"
